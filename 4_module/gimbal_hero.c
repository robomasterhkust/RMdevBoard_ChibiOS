...
static THD_FUNCTION(gimbal_thread, p)
{
    ...
    _yaw_vel.
            error_int_max = 2000.0f;
    _pitch_vel.
            error_int_max = 2500.0f;
    _yaw_atti.
            error_int_max = 4.0f;
    _pitch_atti.
            error_int_max = 4.0f;
    ...
    while (!
        ...
        gimbal.
                d_yaw = gimbal.motor[GIMBAL_YAW]._angle - yaw_init_pos;
 ...
 */
        ...
        gimbal.motor[GIMBAL_PITCH].
                _speed = gimbal._pIMU->gyroData[Y];
        gimbal.motor[GIMBAL_YAW].
                _speed = pGyro->angle_vel * cosf(yaw_theta1) -
...
//TODO Yaw mixer is not aligned
        ...
        gimbal.motor[GIMBAL_YAW].
                _speed_cmd = -sinroll * pitch_atti_out + cospitch * cosroll * yaw_atti_out;

        if (
                cosf(yaw_theta1)
                > 0.1f)
            gimbal.motor[GIMBAL_YAW]._speed_cmd /=
                    cosf(yaw_theta1);
        else
            gimbal.motor[GIMBAL_YAW].
                    _speed_cmd = 0.0f;
        ...
        gimbal.motor[GIMBAL_PITCH].
                _speed_cmd = cosroll * pitch_atti_out + cospitch * sinroll * yaw_atti_out;

        gimbal.
                yaw_iq_cmd = gimbal_controlSpeed(&_yaw_vel, &gimbal.motor[GIMBAL_YAW]);
        gimbal.
                pitch_iq_cmd = gimbal_controlSpeed(&_pitch_vel, &gimbal.motor[GIMBAL_PITCH]);
/*
...
//Moment of inertia decreases as theta1 moves away from 0
        gimbal.yaw_iq_cmd -=
                (
                        ...
                        *
                        sinf(yaw_theta2)
                        * gimbal.axis_ff_ext[2]) * gimbal.
                        yaw_iq_cmd;

/*output limit*/
        gimbal.
                yaw_iq_output = boundOutput(gimbal.yaw_iq_cmd, GIMBAL_IQ_MAX);
        gimbal.
                pitch_iq_output = boundOutput(gimbal.pitch_iq_cmd, GIMBAL_IQ_MAX);
...
#define GIMBAL_INIT_MAX_ERROR         1e-1 // 2e-2
#define GIMBAL_INIT_SCORE_FULL        100U
static THD_WORKING_AREA(gimbal_init_thread_wa,
                        2048);
...
)
{
    ...
    _pitch_pos.
            error_int_max = 1500.0f;
    _yaw_pos.
            error_int_max = 1000.0f;
    ...
    while (!
        ...
        _error[GIMBAL_YAW] = gimbal.axis_init_pos[GIMBAL_YAW] - gimbal.motor[GIMBAL_YAW].
                _angle;
        _error[GIMBAL_PITCH] = gimbal.axis_init_pos[GIMBAL_PITCH] - gimbal.motor[GIMBAL_PITCH].
                _angle;

        while (_error[GIMBAL_YAW] > M_PI)
            _error[GIMBAL_YAW] -= 2 *
                                  M_PI * GIMBAL_YAW_GEAR;
        while (_error[GIMBAL_YAW] < -M_PI)
            _error[GIMBAL_YAW] += 2 *
                                  M_PI * GIMBAL_YAW_GEAR;

        while (_error[GIMBAL_PITCH] > M_PI)
            _error[GIMBAL_PITCH] -= 2 *
                                    M_PI;
        while (_error[GIMBAL_PITCH] < -M_PI)
            _error[GIMBAL_PITCH] += 2 *
                                    M_PI;
        ...
        if (
                ...
                > tick)
            gimbal.
                    yaw_iq_cmd = gimbal_controlPos(&_yaw_pos, _error[GIMBAL_YAW],
                                                   gimbal.motor[GIMBAL_YAW]._speed_enc);

        gimbal.
                pitch_iq_cmd = gimbal_controlPos(&_pitch_pos, _error[GIMBAL_PITCH],
                                                 gimbal.motor[GIMBAL_PITCH]._speed_enc);

        gimbal.
                yaw_iq_output = gimbal.yaw_iq_cmd;
        gimbal.
                pitch_iq_output = gimbal.pitch_iq_cmd;
