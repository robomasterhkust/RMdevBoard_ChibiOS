...
uint16_t JudgementDataRetrievedType[] = {
        ...
        CAN_CHASSIS_BOARD_PROJECTILE_HLTH_ID,
        CAN_CHASSIS_BOARD_POWER_POWERBUFFER_ID,
        CAN_CHASSIS_BOARD_VOLT_CURRENT_ID,
        CAN_CHASSIS_BOARD_SHOOTERHEAT_RFID_BUFFERINFO_ID,
...
static inline void can_processSendJudgeData(volatile Chassis_Send_Judge_canStruct *msg, const CANRxFrame *const rxmsg) {
    ...
    switch (rxmsg->SID) {
        case CAN_CHASSIS_BOARD_GAMEINFO_ID:
            msg->gameInfo.remainTime = (uint16_t) rxmsg->data16[0];
        ...
        case CAN_CHASSIS_BOARD_PROJECTILE_HLTH_ID:
            ...
            msg->projectileInfo.bulletType = (uint8_t) rxmsg->data8[4];
            msg->projectileInfo.bulletFreq = (uint8_t) rxmsg->data8[5];
            msg->hlthInfo.hitPos = (uint8_t) rxmsg->data8[6];
            msg->hlthInfo.deltaReason = ((uint8_t) rxmsg->data8[7]);
            break;
        case CAN_CHASSIS_BOARD_POWER_POWERBUFFER_ID:
            msg->powerInfo.power = (float) rxmsg->data32[0];//* 0.00001f;
        ...
        case CAN_CHASSIS_BOARD_VOLT_CURRENT_ID:
            msg->powerInfo.volt = (float) rxmsg->data32[0];//* 0.00001f;
            msg->powerInfo.current = (float) rxmsg->data32[1];//* 0.00001f;
            break;
        case CAN_CHASSIS_BOARD_SHOOTERHEAT_RFID_BUFFERINFO_ID:
            msg->powerInfo.shooterHeat0 = (uint16_t) rxmsg->data16[0];
            msg->powerInfo.shooterHeat1 = (uint16_t) rxmsg->data16[1];
            msg->rfidInfo.cardType = (uint8_t) rxmsg->data8[4];
            msg->rfidInfo.cardIdx = (uint8_t) rxmsg->data8[5];
            msg->bufferInfo.powerUpType = (uint8_t) rxmsg->data8[6];
            msg->bufferInfo.powerUpPercentage = ((uint8_t) rxmsg->data8[7]);
            break;
        case CAN_CHASSIS_BOARD_LOCATION_X_Y_ID:
            msg->locationInfo.x = (float) rxmsg->data32[0];//* 0.00001f;
            msg->locationInfo.y = (float) rxmsg->data32[1];//* 0.00001f;
            break;
        case CAN_CHASSIS_BOARD_LOCATION_Z_YAW_ID:
            msg->locationInfo.z = (float) rxmsg->data32[0];//* 0.00001f;
            msg->locationInfo.yaw = (float) rxmsg->data32[1];//* 0.00001f;
            break;
    }
    chSysUnlock();
}
...
void JudgeData_txCan(judge_fb_t *JudgeData, CANDriver *const CANx, const uint16_t SID) {
    ...
    typedef struct projectile_hlth_fb_t {
        float bulletSpeed;
        uint8_t bulletType;
        uint8_t bulletFreq;
        uint8_t hitPos : 4;
        uint8_t deltaReason : 4;
    }__attribute__((packed)) projectile_hlth_fb_t;
    ...
    typedef struct power_fb_t3_rfid_buffer {
    ...
    }__attribute__((packed)) power_fb_t3_rfid_buffer;
    ...
    for (i = 0; i < 7; i++) {
        ...
        switch (JudgementDataRetrievedType[i]) {
            ...
            case CAN_CHASSIS_BOARD_PROJECTILE_HLTH_ID: {
                projectile_hlth_fb_t txCan2;
                txCan2.bulletType = JudgeData->projectileInfo.bulletType;
                txCan2.bulletFreq = JudgeData->projectileInfo.bulletFreq;
                txCan2.bulletSpeed = JudgeData->projectileInfo.bulletSpeed;//* 0.00001f;
            ...
            case CAN_CHASSIS_BOARD_POWER_POWERBUFFER_ID: {
                power_fb_t1 txCan3;
                txCan3.power = JudgeData->powerInfo.power;//* 0.00001f;
                txCan3.powerBuffer = JudgeData->powerInfo.powerBuffer;//* 0.00001f;
                memcpy(&(txmsg.data8), &txCan3, 8);
                break;
            }
            case CAN_CHASSIS_BOARD_VOLT_CURRENT_ID: {
                power_fb_t2 txCan4;
                txCan4.volt = JudgeData->powerInfo.volt;//* 0.00001f;
                txCan4.current = JudgeData->powerInfo.current;//* 0.00001f;
                memcpy(&(txmsg.data8), &txCan4, 8);
                break;
            }
            case CAN_CHASSIS_BOARD_SHOOTERHEAT_RFID_BUFFERINFO_ID: {
                power_fb_t3_rfid_buffer txCan5;
                txCan5.shooterHeat0 = JudgeData->powerInfo.shooterHeat0;
                txCan5.shooterHeat1 = JudgeData->powerInfo.shooterHeat1;
                txCan5.cardType = JudgeData->rfidInfo.cardType;
                txCan5.cardIdx = JudgeData->rfidInfo.cardIdx;
                txCan5.powerUpType = JudgeData->bufferInfo.powerUpType;
                txCan5.powerUpPercentage = JudgeData->bufferInfo.powerUpPercentage;
                memcpy(&(txmsg.data8), &txCan5, 8);
                break;
            }
            case CAN_CHASSIS_BOARD_LOCATION_X_Y_ID: {
                location_fb_t1 txCan6;
                txCan6.x = JudgeData->locationInfo.x;//* 0.00001f;
                txCan6.y = JudgeData->locationInfo.y;//* 0.00001f;
                memcpy(&(txmsg.data8), &txCan6, 8);
                break;
            }
            case CAN_CHASSIS_BOARD_LOCATION_Z_YAW_ID: {
                // LEDB_ON();
                location_fb_t2 txCan7;
                txCan7.z = JudgeData->locationInfo.z;//* 0.00001f;
                txCan7.yaw = JudgeData->locationInfo.yaw;//* 0.00001f;
