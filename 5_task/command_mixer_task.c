...
static THD_FUNCTION(command_mixer, ptr)
{
    ...
    {
        // handle joystick
        gimbal_cmd.yaw_vel_cmd = map(pRC->rc.channel3, RC_CH_VALUE_MIN, RC_CH_VALUE_MAX, (float)M_PI/2000, (float)-M_PI/2000);
        gimbal_cmd.pitch_vel_cmd = map(pRC->rc.channel2, RC_CH_VALUE_MIN, RC_CH_VALUE_MAX, (float)M_PI/700, (float)-M_PI/700);
