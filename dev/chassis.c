...
bool chassis_absolute_speed(float i) {
  float rpm = fabsf(chassis._motors[0]._speed) / 4 +
              fabsf(chassis._motors[1]._speed) / 4 +
              fabsf(chassis._motors[2]._speed) / 4 +
              fabsf(chassis._motors[3]._speed) / 4;
  if (rpm < 100 * i) {
    return true;
  }
  return false;
  ...
  while (!chThdShouldTerminateX()) {
    ...
    */
    if (JudgeP->powerInfo.powerBuffer <= 30) {
      ...
      if (chassis.power_limit <= 25 && JudgeP->powerInfo.power > 100 &&
          !chassis_absolute_speed(1)) {
