...
#define accl_value (165.0f / 500.0f) // 500 is the frequency and 1 means 1 second
                                       ...
                                       float vx, float vy, float w_degree)
{
    ...
    float dx = chassisPtr->rotate_x_offset;
    float dy = chassisPtr->rotate_y_offset;
...
static void chassis_encoderUpdate(void)
{
    ...
    for (i = 0; i < CHASSIS_MOTOR_NUM; i++) {
        if (chassis._encoders[i].updated) {
            ...
            float speed_input = chassis._encoders[i].raw_speed / MOTOR_GEAR_RATIO;
 ...
 * motor speed in RPM, rotation per minute
 * speed command in RPM, rotation per minute
 */
static int16_t chassis_controlSpeed(motorStruct *motor,
                                    pi_controller_t *controller)
{
    float error = motor->speed_curve - motor->_speed;
    ...
    return (int16_t) (boundOutput(output, CHASSIS_OUTPUT_MAX));
}
...
{
    float rpm = fabsf(chassis._motors[0]._speed) / 4 + fabsf(chassis._motors[1]._speed) / 4 +
                fabsf(chassis._motors[2]._speed) / 4 + fabsf(chassis._motors[3]._speed) / 4;
    if (rpm > 125 * i) {
        return true;
    }
    return false;
...
{
    float Collision_extent =
            (IMU_Data->accelData[0] * IMU_Data->accelData[0] + IMU_Data->accelData[1] * IMU_Data->accelData[1]) /
            sqrt(IMU_Data->accelData[0] * IMU_Data->accelData[0] + IMU_Data->accelData[1] * IMU_Data->accelData[1]);
...
/**
 * Send chassis speed and speed command to ROS through CAN bus
 * @param CANx
 */
...
void mecanum_cal()
{
    ...
    if (fabsf(chassis.strafe_curve) <= fabsf(chassis.strafe_sp)) {
    ...
    } else if (fabsf(chassis.strafe_curve) > fabsf(chassis.strafe_sp)) {
        // if(fabsf(chassis.strafe_sp) < 0.003){ // check whether the user intended
        // to stop
        ...
        if (fabsf(chassis.strafe_sp) < 0.003) {
    ...
    if (fabsf(chassis.drive_curve) <= fabsf(chassis.drive_sp)) {
    ...
    } else if (fabsf(chassis.drive_curve) > fabsf(chassis.drive_sp)) {
        ...
        if (fabsf(chassis.drive_sp) < 0.003) {
...
void follow_gimbal_handle()
{
    ...
    chassis.rotate_sp = boundOutput(chassis_heading_control(
            &chassis_heading_controller, gimbal_p[0].radian_angle, gimbal_initP), MAX_CHASSIS_VR_SPEED);
}
        ...
        if (JudgeP->powerInfo.powerBuffer <= 40) {
            ...
            for (i = 0; i < 4; i++) {
                if (chassis._motors[i].speed_sp >= chassis._motors[i]._speed &&
                    ...
                    fabsf(chassis._motors[i]._speed)) {
            ...
            for (i = 0; i < 4; i++) {
                if (chassis._motors[i].speed_sp > chassis._motors[i].speed_curve &&
                    fabsf(chassis._motors[i].speed_sp) >
                    fabsf(chassis._motors[i].speed_curve)) {
                    chassis._motors[i].speed_curve += accl_value;
...
void power_limit_handle()
{
    ...
    float a0 = fabsf(chassis._encoders[0].act_current * 20.0/16384.0);
    float a1 = fabsf(chassis._encoders[1].act_current * 20.0/16384.0);
    ...
    float A3 = fabsf(chassis.current[3]* 20.0/16384.0);
